        ...
        .toolbar {
            ToolbarItem(placement: .topBarLeading) {
                Button {
                    ...
                    DesignSystemAsset.Image.iconArrowBack.swiftUIImage
    ...
    private var matchingButtonView: some View {
        HStack(alignment: .bottom, spacing: 0) {
            VStack(spacing: 0) {
                ...
                .buttonStyle(.funch(isEnabled: viewModel.isEnabled))
                .shadow(
                    color: viewModel.isEnabled
                    ? DesignSystemAsset.Color.lemon500.swiftUIColor.opacity(0.7)
                    : .clear,
                    radius: 4, x: 0, y: 4
                )
...
extension ProfileEditorView {
    ...
    private var majorInputField: some View {
        DynamicHGrid(itemSpacing: 8, lineSpacing: 8) {
            ForEach(Profile.Major.examples, id: \.self) { major in
                ChipButton(
                    action: {
    ...
    private var clubsInputField: some View {
        DynamicHGrid(itemSpacing: 8, lineSpacing: 8) {
            ForEach(Profile.Club.examples, id: \.self) { club in
                ChipButton(
                    action: {
